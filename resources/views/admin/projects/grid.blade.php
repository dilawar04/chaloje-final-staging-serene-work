@php
            $status_column_data = DB_enumValues('projects', 'status');
...
<form action="{{ admin_url('', true) }}" method="get" enctype="multipart/form-data" id="projects-form">
    @csrf
    @include('admin.layouts.inc.stickybar', compact('form_buttons'))
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        <div class="row">
            <div class="col-lg-12">
                <div class="kt-portlet" data-ktportlet="true" id="kt_portlet_projects">
                        ...
                        $grid->dt_column(['created' => ['input_options' => ['class' => 'm_datepicker']]]);
                            ...
                            'wrap' => function($value, $field, $_row) use($_this) {
                                return grid_img(asset_url("{$_this->module}/{$value}"), 48, 48);
                                                ...
                                                $grid->dt_column(['project_map' => ['align' => 'center',
                            'wrap' => function($value, $field, $_row) use($_this) {
                                return grid_img(asset_url("{$_this->module}/{$value}"), 48, 48);
                            }
                        ]]);
