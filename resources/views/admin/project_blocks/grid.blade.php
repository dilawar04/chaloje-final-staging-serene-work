@php
            $status_column_data = DB_enumValues('project_blocks', 'status');
...
<form action="{{ admin_url('', true) }}" method="get" enctype="multipart/form-data" id="project_blocks-form">
    @csrf
    @include('admin.layouts.inc.stickybar', compact('form_buttons'))
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        <div class="row">
            <div class="col-lg-12">
                <div class="kt-portlet" data-ktportlet="true" id="kt_portlet_project_blocks">
                    ...
                    <div class="kt-portlet__body kt-padding-0">
                        ...
                        $grid->dt_column(['floor_plan' => ['align' => 'center',
                            'wrap' => function($value, $field, $_row) use($_this) {
                                return grid_img(asset_url("{$_this->module}/{$value}"), 48, 48);
                            }
                        ]]);
                        $grid->dt_column(['payment_plan' => ['align' => 'center',
                            'wrap' => function($value, $field, $_row) use($_this) {
                                return grid_img(asset_url("{$_this->module}/{$value}"), 48, 48);
                            }
                        ]]);
