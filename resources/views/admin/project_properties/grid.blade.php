@php
    $dealers = \App\User::where(['user_type_id' => 6])->pluck('first_name', 'id')->toArray();

    //$status_column_data = DB_enumValues('project_properties', 'status');
    $status_column_data = \App\ProjectProperty::select(['status'])->groupBy('status')->pluck('status')->toArray();
...
@section('content')
    ...
    <form action="{{ admin_url('', true) }}" method="get" enctype="multipart/form-data" id="project_properties-form">
        @csrf
        @include('admin.layouts.inc.stickybar', compact('form_buttons'))
        <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
            <div class="row">
                <div class="col-lg-12">
                    <div class="kt-portlet" data-ktportlet="true" id="kt_portlet_project_properties">
                        ...
                        <div class="kt-portlet__body kt-padding-0">
                            @php
                                ...
                                $grid->dt_column(['status' => ['overflow' => 'initial', 'align' => 'center', 'width' => '150', 'th_align' => 'center', 'filter_value' => '=', 'input_options' => ['options' => $grid->status_column_data, 'class' => '', 'onchange' => true],
                                ...
                                $grid->dt_column(['dealer' => ['width' => '200', 'th_align' => 'center',
                                    'wrap' => function($value, $field, $_row, $grid) use($dealers){
                                        return "<select name='dealer_id[".$_row['id']."]' class='m-select2 update-row' style='width: 100%;' data-url='".admin_url("ajax/dealer/{$_row['id']}", true)."'>
                                        <option>Assign to dealer</option>
                                        ".selectBox($dealers, $value)."
                                        </select>";
                                    }
                                ]]);
