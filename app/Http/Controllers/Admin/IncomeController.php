...
class IncomeController extends Controller
{
    ...
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'income.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "income.id
, TRIM(CONCAT_WS(' ', users.first_name, users.last_name)) AS customer
-- , orders.id as order_id
, income.rel_id as order_id
, income.title
, income.income_head
, income.date
, income.amount
, income.status
    ...
    ";
        ...
        $SQL = $SQL->leftJoin('users', 'users.id', '=', 'income.user_id');
        $SQL = $SQL->leftJoin('orders', 'orders.id', '=', 'income.rel_id');

        //$SQL = $SQL->leftJoin('users', 'users.id', '=', 'income.created_by');
        /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method income store | Insert & Update
     * *****************************************************************************************************************
     */
    public function store()
    {
        ...
        if($order_id > 0){
            ...
            req('user_id', null, $order->customer_id);
            req('rel_id', null, $order->id);
        }
        ...
        /** -------- Upload Files */
        $files = upload_files(['receipt'], "assets/front/{$this->table}/");
        if (count($files) > 0) {
        ...
        if($order_id > 0){
            ...
            $_data = \App\Order::mail_data($order_id);
            $mail_temp = \App\EmailTemplate::template($_data, "Payment {$data['status']}");
            if ($mail_temp->status == 'Active') {
                ...
                if (send_mail($mail_data)) {
                    set_notification('Email has been sent to: ' . $mail_data['to']);
                } else {
                    set_notification('Email sending failed to: ' . $mail_data['to']);
                }
            }
        }
        ...
        if (request()->ajax() || request()->is('api/*')) {
            ...
            if($order_id > 0) return redirect()->back();
