...
class OrderController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method orders index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => 'orders.' . $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'orders.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "orders.id
, orders.order_number
, CONCAT_WS(' ', users.first_name, users.last_name) as customer
, orders.status
...
, orders.created_at
    ";
        ...
        $SQL = $SQL->leftJoin('users', 'users.id', '=', 'orders.customer_id');
        //$SQL = $SQL->leftJoin('order_statuses', 'order_statuses.id', '=', 'orders.status');
        //$SQL = $SQL->leftJoin('users', 'users.id', '=', 'orders.created_by');
        /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method orders store | Insert & Update
     * *****************************************************************************************************************
     */
    public function store()
    {
        ...
        /** -------- Validation */
        $validator_rules = [
            'order_number' => "required|unique:{$this->table},order_number,{$id},{$this->id_key}",
        ];
