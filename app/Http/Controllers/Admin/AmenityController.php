...
class AmenityController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method amenities index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'amenities.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "*
-- , amenities_groups.group_id
    ";
        $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('amenities_groups', 'amenities_groups.id', '=', 'amenities.group_id');
        /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method amenities store | Insert & Update
     * *****************************************************************************************************************
     */
    public function store()
    {
        ...
        $validator_rules = [
            'title' => "required",
            'code' => "required|unique:{$this->table},code,{$id},{$this->id_key}",
        ...
        /** -------- Upload Files */
        $files = upload_files(['icon'], "assets/front/{$this->table}/");
        if (count($files) > 0) {
    ...
    public function view()
    {
        ...
        $config['image_fields'] = [
            'icon' => ['path' => asset_url("{$this->module}"), 'size' => '128x128'],
        ];
        $config['attributes'] = [
            'id' => ['title' => 'ID'],
        ];
    ...
    public function file_upload()
    {
        ...
        $files = upload_files(['icon'], $dir, ['ext' => 'gif,jpg,jpeg,png']);
        if (count($files) > 0) {
            foreach ($files as $name => $file) {
                if ($file) {
