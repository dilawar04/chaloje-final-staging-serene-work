...
/**
 * Class PropertyType
 * @property \App\PropertyType $model
 */
...
class PropertyTypeController extends Controller
{
    ...
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'property_types.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "*
-- , property_types.parent_id
    ";
        $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('property_types', 'property_types.id', '=', 'property_types.parent_id');
                /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method property_types store | Insert & Update
     * *****************************************************************************************************************
     */
    ...
    public function view()
    {
        ...
        $config['image_fields'] = [
                            'image' => ['path' => asset_url("{$this->module}"),  'size' => '128x128'],
                    ];
        $config['attributes'] = [
            'id' => ['title' => 'ID'],
        ];
    ...
    /**
     * *****************************************************************************************************************
     * @method property_types duplicate
     * *****************************************************************************************************************
     */
