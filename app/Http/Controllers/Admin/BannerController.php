...
class BannerController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method banners index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'banners.' . $this->id_key])->merge(request()->query())->toArray();
...
, banners.status
            ...
            $response = collect($paginate_OBJ)->map(function ($data, $key){
                if($key == 'data'){
                    return collect($data)->map(function ($row){
                        return collect($row)->mapWithKeys(function ($dd) use ($row){
                            return collect($row)->merge(['imageURL' => (asset_url("{$this->module}/{$row['image']}")) ]);
                        });
                    });
                }
                return $data;
    ...
    /**
     * *****************************************************************************************************************
     * @method banners store | Insert & Update
     * *****************************************************************************************************************
     */
    public function store()
    {
        ...
        $validator_rules = [
            //'image' => "required",
            //'image' => "image|mimes:gif,jpg,jpeg,png|max:2048",
        ...
        /** -------- Upload Files */
        $files = upload_files(['image'], "assets/front/{$this->table}/");
        if (count($files) > 0) {
    ...
    public function delete()
    {
        ...
        $unlink = ['image' => "assets/front/{$this->table}",
        ];
    ...
    public function file_upload()
    {
        ...
        $files = upload_files(['image'], $dir, ['ext' => 'gif,jpg,jpeg,png']);
        if (count($files) > 0) {
            foreach ($files as $name => $file) {
                if ($file) {
