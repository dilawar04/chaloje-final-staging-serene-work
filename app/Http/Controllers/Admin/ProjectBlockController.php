...
/**
 * Class ProjectBlock
 * @property \App\ProjectBlock $model
 */
...
class ProjectBlockController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method project_blocks index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'project_blocks.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "project_blocks.id
-- , project_blocks.project_id
, projects.title as project
, project_blocks.title
, project_blocks.area
, project_blocks.area_unit
-- , project_blocks.square_meter
, project_blocks.floor_plan
, project_blocks.payment_plan
, project_blocks.status
    ";
        $SQL = $this->model->select(\DB::raw($select));
        $SQL = $SQL->leftJoin('projects', 'projects.id', '=', 'project_blocks.project_id');
        /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method project_blocks store | Insert & Update
     * *****************************************************************************************************************
     */
    public function store()
    {
        ...
        $validator_rules = [
            'project_id' => "required",
            'title' => "required",
            //'floor_plan' => "image|mimes:gif,jpg,jpeg,png|max:2048",
        ...
        /** -------- Upload Files */
        $files = upload_files(['floor_plan', 'payment_plan'], "assets/front/{$this->table}/");
        if (count($files) > 0) {
        ...
        if ($status = $row->save()) {
             ...
             * | Amenities
             *‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒*/
            \DB::table('block_amenities')->where(['block_id' => $id])->delete();
            ...
            if (count($amenities) > 0) {
                foreach ($amenities as $amenity_id => $amenity) {
                    ...
                    if(!empty($amenity)) {
                        \DB::table('block_amenities')->insert($am_data);
                    }
                }
            }
        } else {
    ...
    public function delete()
    {
        ...
        $unlink = ['floor_plan' => "assets/front/{$this->table}",
            'payment_plan' => "assets/front/{$this->table}",
        ];
    ...
    /**
     * *****************************************************************************************************************
     * @method project_blocks view | Record
     * *****************************************************************************************************************
     */
    public function view()
    {
        ...
        $config['image_fields'] = [
            'floor_plan' => ['path' => asset_url("{$this->module}"), 'size' => '128x128'],
            'payment_plan' => ['path' => asset_url("{$this->module}"), 'size' => '128x128'],
        ];
        $config['attributes'] = [
            'id' => ['title' => 'ID'],
        ];
    ...
    /**
     * *****************************************************************************************************************
     * @method project_blocks AJAX actions
     * *****************************************************************************************************************
     */
    function ajax()
    {
    ...
    /**
     * *****************************************************************************************************************
     * @method project_blocks duplicate
     * *****************************************************************************************************************
     */
    ...
    public function file_upload()
    {
        ...
        $files = upload_files(['floor_plan', 'payment_plan'], $dir, ['ext' => 'gif,jpg,jpeg,png']);
        if (count($files) > 0) {
            foreach ($files as $name => $file) {
                if ($file) {
