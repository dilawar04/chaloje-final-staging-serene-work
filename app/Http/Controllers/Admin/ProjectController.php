...
class ProjectController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method projects index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'projects.' . $this->id_key])->merge(request()->query())->toArray();
...
, projects.status
    ...
    ";
        ...
        //$SQL = $SQL->leftJoin('countries', 'countries.id', '=', 'projects.country');
        $SQL = $SQL->leftJoin('cities', 'cities.id', '=', 'projects.city_id');
        //$SQL = $SQL->leftJoin('areas', 'areas.id', '=', 'projects.area_id');
        //$SQL = $SQL->leftJoin('users', 'users.id', '=', 'projects.developer_id');
        //$SQL = $SQL->leftJoin('users', 'users.id', '=', 'projects.created_by');
        /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method projects store | Insert & Update
     * *****************************************************************************************************************
     */
    public function store()
    {
        ...
        $validator_rules = [
            ...
            'country' => "required",
            'city_id' => "required",
            'project_map' => "image|mimes:gif,jpg,jpeg,png|max:5120",
        ...
        /** -------- Upload Files */
        $files = upload_files(['logo', 'project_map', 'master_plan'], "assets/front/{$this->table}/");
        if (count($files) > 0) {
        ...
        if ($status = $row->save()) {
             ...
             * | Amenities
             *‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒*/
            \DB::table('project_amenities')->where(['project_id' => $id])->delete();
            $amenities = req('amenities');
            if (count($amenities) > 0) {
                foreach ($amenities as $amenity_id => $amenity) {
                    $am_data = [];
                    $am_data['project_id'] = $id;
                    $am_data['amenity_id'] = $amenity_id;
                    $am_data['amenity_value'] = $amenity;
                    if(!empty($amenity)) {
                        \DB::table('project_amenities')->insert($am_data);
                    }
                }
            }
        } else {
    ...
    public function delete()
    {
        ...
        $unlink = ['logo' => "assets/front/{$this->table}",
            'project_map' => "assets/front/{$this->table}",
        ];
    ...
    public function view()
    {
        ...
        $config['image_fields'] = [
            'logo' => ['path' => asset_url("{$this->module}"), 'size' => '128x128'],
            'project_map' => ['path' => asset_url("{$this->module}"), 'size' => '128x128'],
        ];
        $config['attributes'] = [
            'id' => ['title' => 'ID'],
        ];
    ...
    public function file_upload()
    {
        ...
        $files = upload_files(['logo', 'project_map', 'file'], $dir, ['ext' => 'gif,jpg,jpeg,png']);
        if (count($files) > 0) {
            foreach ($files as $name => $file) {
                if ($file) {
