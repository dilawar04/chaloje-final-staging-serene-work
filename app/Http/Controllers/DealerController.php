...
class DealerController extends Controller
{
    ...
    public function login()
    {
        if(\request()->method() == 'POST'){
            ...
            if (\Auth::attempt($credentials, intval(req('remember')))) {
                ...
                $redirect = str_replace(url(''), '', redirect()->intended(url('dealer/account'))->getTargetUrl());
        ...
        return \App\Theme::view('dealer/login', compact('page'));
    }
    ...
    public function registration()
    {
        ...
        $page = [];
        \App\Theme::template($page->template ?? 'default');
        return \App\Theme::view('dealer/register', compact('page'));
    }
    ...
    function logout()
    {
        ...
        return \Redirect::to(url('dealer/login'));
    }
    ...
    function edit_profile()
    {
        if(!\Auth::check()) return redirect()->to('/dealer/login');
        $user = \Auth::user();
        if($user->id > 0 && $user->user_type_id == 6) {
            ...
            if(\request()->isMethod('POST')){
                ...
                $rel_DB_data = DB_FormFields('dealer_rel', [], \request()->input('rel'))['data'];
                \DB::table('dealer_rel')->updateOrInsert(['user_id' => $user->id], $rel_DB_data);
                if($affected) {
            ...
            $rel = \DB::table('dealer_rel')->where(['user_id' => $user->id])->first();
            $user = json_decode(collect($user)->merge($rel)->toJson());

            \App\Theme::template($page->template ?? 'default');
            return \App\Theme::view('dealer/edit_profile', compact('user'));
        }
    }
    ...
    public function account(){
        ...
        \App\Theme::template($page->template ?? 'default');
        return \App\Theme::view('dealer/dashboard', compact('page', 'user'));
    }
