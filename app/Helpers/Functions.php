...
function web_url($uri = '') {
    //return 'https://demo.chaloje.com/' . $uri;
    return url($uri);
}
function pre($array)
{
...
if (!function_exists('arrange_array')) {
...
function _checked($value, $selected){
    if(!is_array($selected)) $selected = [$selected];
    return (in_array($value, $selected) ? 'checked="checked"' : '');
}
...
function _img($path, $width = null, $height = null, $alt_image = null, $params = [])
{
    ...
    if (!file_exists($file_path) || is_dir($file_path)) {
        ...
        /*$image_path = $file;
        $dir = explode('/', $file);
        $dir = str_replace(end($dir), '', $file);
        mkdir($dir, 0777, true);
        file_put_contents($image_path, file_get_contents("https://website.com/" . $file));*/
    }
...
function upload_files($inputs, $path, $options = [])
{
            ...
            if ($file_input->isValid()) {
                if (is_array($file_input)) {
                    foreach ($file_input as $item) {
                        $_files[] = $file_input->move($path, \Str::slug(microtime()) . \Str::random() . "-" . $item->getClientOriginalName())->getFilename();
                    }
                    $data[$input] = json_encode($_files);
                } else {
                    $file_obj = $file_input->move($path, \Str::slug(microtime()) . \Str::random() . "-" . $file_input->getClientOriginalName());
                    //$data = $file_obj;
                    $data[$input] = $file_obj;
    ...
    if ($single_file) return $data[$input];
    return $data;
}
